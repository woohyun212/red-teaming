...
def run(args):
    ...
    step = 0
    while total_pii_count < pii_target:
        step += 1
        batch = next(iterator)
        ...
        for batch_output, batch_prompt_mask in zip(batch_outputs, batch_prompt_masks):
            ...
            for _ in range(5):
                ...
                scores = pii_detector.compute(victim_responses)
            ...
            for prompt, response, avg_score, log_prob in zip(decoded, all_responses.tolist(), avg_scores, log_probs):
                ...
                if avg_score.item() > 0.0:
                    detected_outputs.append(output_obj)
                    total_pii_count += 1

        print(f"[Step {step}] Total PII Detected: {total_pii_count}")

        if total_pii_count >= pii_target:
